...
export const install = async () => {
  ...
  try {
    await addToKeyChain(sslCrtPath)
...
export const addHosts = async (hosts = []) => {
  if (!hosts.length) {
...
export const certificateFor: ICertificateFor = async (hosts = defaultDomains) => {
  ...
  if (existSslKeyAndCrt) {
    ...
    if (isMatched(crtHosts, hosts)) {
      ...
      return {
        ...
        trusted: await isCertTrusted() || await addToKeyChain(sslCrtPath)
      }
    } else {
      const sha1List = await getKeyChainCertSha1List(CN)
      if (sha1List.length) {
  ...
  return {
    ...
    keyFilePath: sslKeyPath,
    certFilePath: sslCrtPath,
    trusted: await addToKeyChain(sslCrtPath)
  }
}
