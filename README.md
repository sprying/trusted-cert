# 简介
HTTPS自签名证书工具，是为了解决启动本地HTTPS服务时，需要手动创建自签名证书的繁琐。使用命令行实现一键生成自签名证书并添加到系统钥匙串，后续使用命令可以继续查看和管理证书，同时提供了API，方便地集成到工程工具里。
...
1. **生成密钥和证书** 创建过程会提示输入域名，这里使用默认域名，直接回车；输入密码，向macOS钥匙串里添加证书。
	...
	```bash
	# 全局安装
	npm install httpscert -g
	# 或者使用yarn
	yarn global add httpscert
...
2. 调用api  
	...
	const https = require('https')
	const fs = require('fs')
	const { certificateFor } = require('httpscert')
...
```
Usage: httpscert [global options] command
